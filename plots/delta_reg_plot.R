...
seg_nat_delta <- list(
  list(text=paste0("\u03c1=", fmt2_raw(res_delta_nat$r),
                   ", 95% (CI ", fmt2_raw(unname(ci_nat["lo"])), ", ",
                   fmt2_raw(unname(ci_nat["hi"])), "), "), sub=FALSE),
...
seg_oc_delta <- list(
  list(text=paste0("\u03c1=", fmt2_raw(res_delta_oc$r),
                   ", 95% (CI ", fmt2_raw(unname(ci_oc["lo"])), ", ",
                   fmt2_raw(unname(ci_oc["hi"])), "), "), sub=FALSE),
  list(text="p", sub=FALSE, italic=TRUE),
  list(text="perm", sub=TRUE),
